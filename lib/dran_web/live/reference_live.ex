defmodule DranWeb.ReferenceLive do
  ...
  def render(assigns) do
    ...
    >
      <div :if={@live_action == :show}>
        ...
        >
          <:actions>
            <.link navigate={~p"/panel/references"} class="btn btn-primary btn-sm"><.icon
  ...
  def handle_params(%{"slug" => slug} = params, _url, socket) do
    PageDetail.load_page_detail(socket, params, slug, redirect_to: "/panel/references")
  end
  ...
  def handle_event("show_page", %{"slug" => slug}, socket),
    do: {:noreply, push_navigate(socket, to: ~p"/panel/references/#{slug}")}

  def handle_event("new_page", _params, socket),
    do: {:noreply, push_navigate(socket, to: ~p"/panel/references/new")}
