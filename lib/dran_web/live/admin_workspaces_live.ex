defmodule DranWeb.AdminWorkspacesLive do
  @moduledoc "Workspace management — F3"
  ...
  def mount(_params, session, socket) do
    ...
    socket = assign(socket, active_nav: "admin", page_title: gettext("Workspaces"))
    {:ok, socket}
  end
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope} current_user={@current_user} active_nav="admin">
      <div class="flex-1 overflow-y-auto">
        <div class="w-full p-6 space-y-8">
          ...
          <p class="text-caption">{gettext("Workspace management — F3")}</p>
        </div>
      </div>
    </Layouts.app>
    """
