defmodule DranWeb.AdminUsersLive do
  ...
  def mount(_params, session, socket) do
    ...
    socket = assign(socket, active_nav: "admin", page_title: gettext("Usuarios"))
    {:ok, socket}
  end
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope} current_user={@current_user} active_nav="admin">
      <div class="flex-1 overflow-y-auto">
        <div class="w-full p-6 space-y-8">
          <h1 class="text-title">{gettext("Usuarios")}</h1>
          <p class="text-caption">{gettext("User management — F3")}</p>
        </div>
      </div>
    </Layouts.app>
    """
