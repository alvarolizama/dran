defmodule DranWeb.ArtifactLive do
  @moduledoc "LiveView for artifact pages: index list + detail view with inline editing."
  ...
  def handle_params(%{"slug" => slug} = params, _url, socket) do
    ...
    if context do
      case Brain.get_page_by_slug(slug, context.id) do
        nil ->
          {:noreply, push_navigate(socket, to: ~p"/artifacts")}
  ...
  def handle_event("show_page", %{"slug" => slug}, socket),
    do: {:noreply, push_navigate(socket, to: ~p"/artifacts/#{slug}")}
