defmodule Dran.MCP do
  ...
  alias Dran.{Auth, Knowledge, Repo, Worker}
  ...
  @tools [
    ...
    %{
      ...
      "description" =>
        "Update a note's title, body, or tags. **Meta is MERGED, not replaced** — pass only the fields you want to change; existing meta keys are preserved. This is the key difference from `dran_update_page`, which replaces the entire meta object. Note that `tags` replaces the existing tag list entirely. Link the note to collections separately via dran_create_relation. Returns the updated note's title and slug.",
      "inputSchema" => %{
        "type" => "object",
        "properties" => %{
          "workspace" => %{
