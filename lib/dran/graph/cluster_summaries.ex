defmodule Dran.Graph.ClusterSummaries do
  @moduledoc """
  Generate and query persisted summaries for graph clusters.

  After cluster detection has stamped each page's `meta["cluster_id"]`,
  this module produces a 2-3 sentence natural-language summary per cluster
  (via `Dran.Inference.chat/1`) and stores it alongside the highest-ranked
  pages in a `cluster_summaries` row. The GraphRAG layer reads these back
  to answer questions about a cluster without re-querying the LLM.
  """
  ...
  @doc """
  Generate and persist summaries for all clusters in a context.

  Loads every page in `workspace_id` with `meta->>'cluster_id'` set, groups
  them by cluster, and writes one `ClusterSummary` row per cluster
  (upsert on the `[:workspace_id, :cluster_id]` unique constraint). Inference
  ...
  def generate_all(workspace_id) when is_binary(workspace_id) do
    workspace_id
    |> load_cluster_pages()
    |> Enum.group_by(fn p -> parse_int(p.meta["cluster_id"]) end)
    |> Enum.reject(fn {cid, _pages} -> is_nil(cid) end)
    |> Task.async_stream(
      fn {cluster_id, pages} -> build_summary(workspace_id, cluster_id, pages) end,
  ...
  @doc "Get the summary for a specific cluster."
  @spec get_summary(binary(), integer()) ::
          {:ok, ClusterSummary.t()} | {:error, :not_found}
  def get_summary(workspace_id, cluster_id)
      when is_binary(workspace_id) and is_integer(cluster_id) do
    case Repo.get_by(ClusterSummary, workspace_id: workspace_id, cluster_id: cluster_id) do
      nil -> {:error, :not_found}
      summary -> {:ok, summary}
    end
  end

  @doc "Get the summary for the cluster a page belongs to."
  @spec get_summary_for_page(binary()) :: {:ok, ClusterSummary.t()} | {:error, :not_found}
  def get_summary_for_page(page_id) when is_binary(page_id) do
    page = Repo.get_by(Page, id: page_id)

    with %Page{} <- page,
         cid when is_integer(cid) <- parse_int(page.meta["cluster_id"]),
  ...
  @doc "List all cluster summaries for a context, ordered by page_count desc."
  @spec list_summaries(binary()) :: [ClusterSummary.t()]
  def list_summaries(workspace_id) when is_binary(workspace_id) do
    Repo.all(
      from cs in ClusterSummary,
  ...
  def delete_all(workspace_id) when is_binary(workspace_id) do
    from(cs in ClusterSummary, where: cs.workspace_id == ^workspace_id)
    |> Repo.delete_all()
  ...
  defp load_cluster_pages(workspace_id) do
    Repo.all(
      from p in Page,
        where: p.workspace_id == ^workspace_id,
        where: fragment("meta->>'cluster_id' IS NOT NULL"),
  ...
  defp summarize_with_llm(top) do
    ...
    payload = %{
      ...
      "messages" => [
        ...
        %{"role" => "user", "content" => "Write the cluster summary."}
      ],
      "temperature" => 0.3
    }
  ...
  defp fallback_summary(page_count, top_pages) do
    ...
    "Cluster of #{page_count} pages including: #{titles}"
  end
  ...
  defp upsert_summary(workspace_id, cluster_id, summary, page_count, top_pages) do
    attrs = %{
    ...
    case Repo.insert(
           ClusterSummary.changeset(%ClusterSummary{}, attrs),
           on_conflict: {:replace, [:summary, :page_count, :top_pages, :generated_at]},
           conflict_target: [:workspace_id, :cluster_id]
         ) do
      {:ok, _summary} -> :ok
