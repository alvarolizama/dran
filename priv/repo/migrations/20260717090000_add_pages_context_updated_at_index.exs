defmodule Dran.Repo.Migrations.AddPagesContextUpdatedAtIndex do
  @moduledoc """
  Composite index on pages(context_id, updated_at DESC).

  Optimizes:
  - Brain.stats/1 recent pages query (WHERE context_id = ? ORDER BY updated_at DESC LIMIT 5)
  - Brain.stale_pages/1 (WHERE context_id = ? AND updated_at < ?)
  ...
  def down do
    drop index(:pages, [:context_id, :updated_at], name: :pages_context_updated_at_idx)
  end
end
