defmodule Dran.Repo.Migrations.AddQueryOptimizationIndexes do
  ...
  Cover queries:
    1. `list_pages(workspace_id:, type:, archived:)` — page-type lists
    ...
    3. `graph_data/2` + `top_connected_ids/2` — the global graph joins
       ...
       so `(workspace_id, page_type)` makes the join index-only.
  ...
  def up do
    ...
    create index(:pages, [:workspace_id, :page_type, :archived],
             name: :pages_ctx_type_archived_idx
           )

    # 3: pages by context+type for graph joins / counts.
    create index(:pages, [:workspace_id, :page_type], name: :pages_ctx_type_idx)
    ...
    create index(:brain_log, [:workspace_id, :inserted_at], name: :brain_log_ctx_inserted_at_idx)
  end

  def down do
    drop index(:pages, [:workspace_id, :page_type, :archived], name: :pages_ctx_type_archived_idx)

    drop index(:pages, [:workspace_id, :page_type], name: :pages_ctx_type_idx)

    drop index(:page_versions, [:page_id, :version], name: :page_versions_page_version_uidx)

    drop index(:brain_log, [:workspace_id, :inserted_at], name: :brain_log_ctx_inserted_at_idx)
  end
end
